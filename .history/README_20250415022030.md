...
### 8. **Event List for a Selected Date**
   ...
   - Each event entry includes:
     ...
     - A delete button (Trash icon) for removing the event from the calendar, in case if it is yourself then you can 
     - An edit icon (when applicable) for editing the event.
    - click on the map to see the office address in the google map, edit your entry to delete
